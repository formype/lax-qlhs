...
export function BoardingAttendance() {
  ...
  const filteredData = useMemo(() => {
    ...
    return attendanceData.filter(v => {
      ...
      if (isGiaovienOnly) {
        if (v.className !== teacherClass) return false;
      } else {
        // Target Filter for non-teachers
      ...
      // Boarding Filter
      if (boardingFilter === 'yes' && !v.bantru) return false;
      if (boardingFilter === 'no' && v.bantru) return false;
        ...
        <Card className="filter-card">
          <CardBody>
            <div className="filter-grid">
              ...
              {/* Filter 3: Bán trú */}
              <div className="filter-group">
                <label className="text-sm font-semibold mb-1 block">Bán trú</label>
                <Select 
